"""
SQLAlchemy ORM models for Flask-Migrate schema management.
These models mirror the tables defined in database.py and are used
exclusively by Flask-Migrate to generate and apply Alembic migrations.
The application logic continues to use raw psycopg2 via get_db().
"""
...
class PlanningTheorique(db.Model):
    ...
    date_debut_validite = db.Column(db.Text, nullable=False, server_default='2000-01-01')
...
class CongesCloturesMensuelle(db.Model):
...
class TresorerieCompte(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    compte_num = db.Column(db.Text, nullable=False, unique=True)
